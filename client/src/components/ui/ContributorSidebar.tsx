...
import { CONTRIBUTOR_DASHBOARD_SIDEBAR_LINKS, CONTRIBUTOR_DASHBOARD_SIDEBAR_LINKS_BOTTOM } from '../../consts/Navigation';
  ...
  return (
    <div className='fixed bg-stone-1000 flex flex-col w-60 text-white h-screen border-r-2 border-zinc-900'>
      ...
      <div className='flex-1 pl-2 pt-10'>
        {CONTRIBUTOR_DASHBOARD_SIDEBAR_LINKS.map((item) => (
          <SidebarLink key={item.key} item={item} />
        ))}
      </div>
      <div className='pl-2 pt-10'>
        {CONTRIBUTOR_DASHBOARD_SIDEBAR_LINKS_BOTTOM.map((item) => (
          <SidebarLink key={item.key} item={item} />
        ))}
      </div>
    </div>
